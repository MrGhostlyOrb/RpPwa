...
for(let i = 0; i < productList.data.length; i++){
    ...
    let h2 = "<span class = 'card-title'>" + item.productName + "</span>";
   	let p = "<h6 class = 'card-content'>£" + item.productPrice + "  * inc VAT</h6>";
    ...
    let submit = "<button type = 'sumbit' class = 'btn purple card-button-link' onclick = 'addJson(" + item.productNumber + ");checkBasket()' value = 'Add to Basket' id = 'sub"+item.productNumber+"'>Add to Basket</button>"
    let r = "<button class = 'card-button-link purple btn' value = 'View Product' id = '" + item.productNumber + "rem" + "' onclick = 'location.href = " + loc + "'>" + "View Product</button>";
