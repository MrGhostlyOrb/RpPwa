...
function getProduct(req, resp) {
  ...
  const fileLocation = `/productJson`;
  fetch(fileLocation).then((resp) => {
    if (resp.status !== 200) {
      throw new Error(resp.statusText);
    }
    return resp.json();
  }).then((data) => {
	  console.log(req.body.weather-card)
	  resp.render('productName', {title:'Product 1'})
    setTimeout(() => {
      resp.json(data);
    }, FORECAST_DELAY);
  }).catch((err) => {
...
function onRequest(request,response){
  ...
  app.get('/forecast/:location', getProduct);
  app.get('/forecast/', getProduct);
  app.get('/forecast', getProduct);
