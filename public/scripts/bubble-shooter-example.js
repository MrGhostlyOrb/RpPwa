function sendHighScore(scoreData) {
    console.log('Sending High Score Data');
    fetch('/setHighScore', {
        method: 'post',
        headers: {
    ...
    function stateRemoveCluster(dt) {
        ...
        if (animationstate === 0) {
            ...
            if (floatingclusters.length > 0) {
                ...
                for (i = 0; i<floatingclusters.length; i++) {
                    for (j = 0; j<floatingclusters[i].length; j++) {
                        tile = floatingclusters[i][j];
                        tile.shift = 0;
                        tile.shift = 1;
    ...
    function snapBubble() {
        ...
        if (addtile) {
            ...
            if (checkGameOver()) {
                console.log("Here2");
                console.log(score);
                let elem = document.getElementById('nameModal');
                ...
                modalButton.onclick = ()=>{
                    ...
                    instance.close();
                    let highScoreData = {"Name":playerName, "Score":score};
                    //Send high score data to server
                    sendHighScore(highScoreData);
                    location = location;
                };
...
// Move the rows downwards
    ...
    function getNeighbors(tile) {
        const tilerow = (tile.y + rowoffset) % 2; // Even or odd row
        const neighbors = [];
